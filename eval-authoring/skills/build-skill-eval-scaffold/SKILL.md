...
description: "Creates a realistic project scaffold — a test fixture — for rubric-based effectiveness evaluation of a Claude Code skill. Given a plugin:skill identifier and an optional project description, analyzes the target skill's SKILL.md, references, and dispatched agent definitions to learn what inputs it expects and what signals it looks for, then interviews the user in three phases (technology and shape, signals to plant, file plan) before writing the scaffold to tests/test-suites/{skill}/scaffolds/{name}/. Use when creating, building, or setting up a test scaffold, fixture project, or sample codebase to evaluate a skill's output quality against. Does not create tests.json entries or rubric files — use write-scil-evals or write-skill-eval-rubric for those. Does not create agent scaffolds — use build-agent-eval-scaffold for agents."
argument-hint: "[plugin:skill] [optional description] e.g. example-plugin:code-review for a rails 7 project with postgres"
allowed-tools: Read, Write, Glob, Grep, Bash(mkdir *)
---

Build a realistic project fixture that the test harness runs the target skill against, so an LLM judge can score the skill's output against a rubric. The fixture only works as an eval when it contains signals the target skill is designed to find and reads like code a real developer wrote, so this workflow analyzes the target skill first, then interviews the user in three phases before writing any files.
...
1. **Skill purpose** — a one-sentence summary of what the skill does
2. **Expected inputs** — what file types, config files, and project structure the skill expects to find in a working directory
3. **Signal categories** — the kinds of issues, patterns, or signals the skill detects (grouped into categories)
...
5. **Existing scaffolds** — use Glob on `tests/test-suites/{skill}/scaffolds/*/` and list any found by name so the user can avoid duplicating one; if the suite directory does not exist yet, note that it will be created
6. **Proposed project** — if a description was provided in the arguments, restate the tech stack and project shape it implies; otherwise propose one or two options that fit the skill's expected environment from Step 2
7. **Proposed scaffold name** — derive it from the description: kebab-case with a `-project` suffix (e.g., "rails 7 with postgres" becomes `rails-postgres-project`, "python flask API" becomes `python-flask-project`). If `tests/test-suites/{skill}/scaffolds/{name}/` already exists, say so and ask whether to overwrite it or choose a different name.
...
- Spread signals across multiple source files in a realistic directory structure, and include some files with no intentional signals, BECAUSE real projects mix clean and problematic code; a skill shown only problem files is never tested on telling the two apart.
