...
- Every entry carries an explicit `expect` array with `{ "skill-call": true }` or `{ "skill-call": false }` BECAUSE an omitted `expect` falls back to harness defaults without a warning.
