...
Use this page to query the analytics tables directly: every Parquet file's columns and types, how to join them, and ready-to-run example queries. Skillwalker stores analytics data as eight Parquet files in `analytics/data/`. These are written by `./build/skillwalker update-analytics-data`, which converts JSONL output files to Parquet using DuckDB. The conversion is idempotent — runs already present in the Parquet files are skipped.
...
One row per expectation evaluated. Written by `./build/skillwalker test-eval`.
...
One row per output file captured from the sandbox after a test case runs. Written by `./build/skillwalker test-run` when skills or agents write files to the filesystem.
...
One row per SCIL iteration. Written by `./build/skillwalker scil`.
...
One row per SCIL run. Written by `./build/skillwalker scil`.
...
One row per ACIL iteration. Written by `./build/skillwalker acil`.
...
One row per ACIL run. Written by `./build/skillwalker acil`.
...
Join result rows to test-config via a reconstructed `test_case` key. The normalization matches what Skillwalker writes: spaces become `-`, non-alphanumeric chars (except `-`) are stripped.
