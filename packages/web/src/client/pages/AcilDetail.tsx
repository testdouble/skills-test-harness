...
function SectionHeader({ color, label }: { color: string; label: string }) {
  return (
    <div className="flex items-center gap-3 h-10">
      <span className="w-[3px] h-5 rounded-sm flex-shrink-0" style={{ backgroundColor: color }} />
...
function AccuracyBadge({ label, value, variant }: { label: string; value: number | null; variant: 'train' | 'test' }) {
  ...
  return (
    <span className="text-[14px] font-semibold px-2.5 py-1 rounded" style={{ backgroundColor: bg, color }}>
      {label}: {pct}%
    </span>
  )
}
...
function TrainResultsTable({ results }: { results: AcilTrainResult[] }) {
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="bg-[#1a1b1a] h-[36px] border-b border-[#252625]">
            <th
              className="text-[#4f4f4f] text-[12px] font-bold w-[400px] px-4 py-0 text-left"
              style={{ letterSpacing: '1.5px' }}
            >
              TEST NAME
            </th>
            <th
              className="text-[#4f4f4f] text-[12px] font-bold w-[200px] px-4 py-0 text-left"
              style={{ letterSpacing: '1.5px' }}
            >
              EXPECTED
            </th>
            ...
            <th className="text-[#4f4f4f] text-[12px] font-bold px-4 py-0 text-left" style={{ letterSpacing: '1.5px' }}>
              PASSED
            </th>
          </tr>
        </thead>
        <tbody>
          {results.map((r, i) => (
            ...
            >
              ...
              <td className="text-[#c5c5c5] text-[14px] px-4">{r.expected ? 'delegate' : 'no delegate'}</td>
              <td className="text-[#c5c5c5] text-[14px] px-4">{r.actual ? 'delegate' : 'no delegate'}</td>
...
export function AcilDetail(): JSX.Element {
  ...
  const [details, setDetails] = useState<AcilRunDetails | null>(null)
  ...
  if (error)
    return (
      <div className="px-10 py-8">
        <Link to="/acil" className="flex items-center gap-2 text-[#4f4f4f] text-[18px] mb-4 hover:text-[#f0f0f0]">
          <span>&larr;</span>
          <span>Back to ACIL History</span>
        </Link>
  ...
  return (
    <div className="px-10 py-8">
      {/* Back link */}
      <Link to="/acil" className="flex items-center gap-2 text-[#4f4f4f] mb-6 w-fit hover:text-[#f0f0f0]">
        <span className="text-[20px]">&larr;</span>
        <span className="text-[16px]">Back to ACIL History</span>
      </Link>

      {/* Run header */}
      <div className="flex items-center gap-4 mb-4">
        <span className="text-[#4f4f4f] text-[16px] font-medium">ACIL Run</span>
          ...
          return (
            <div key={iter.iteration} className="mb-5">
              <div className="flex items-center gap-4 mb-3">
                <span className="text-[#f0f0f0] text-[18px] font-bold">Iteration {iter.iteration}</span>
                {isBest && (
              ...
              <p className="text-[#c5c5c5] text-[14px] leading-relaxed whitespace-pre-wrap mb-3">{iter.description}</p>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 h-10 mb-3">
          ...
          <span className="text-[#75fe04] text-[16px] font-semibold">Iteration {summary.bestIteration}</span>
        </div>
        <div className="bg-[#1a1b1a] border-2 border-[#75fe04] rounded-lg p-6">
          <p className="text-[#c5c5c5] text-[15px] leading-relaxed whitespace-pre-wrap">{summary.bestDescription}</p>
        </div>
      </div>
    </div>
  )
