...
export function ScilHistory(): JSX.Element {
  ...
  useEffect(() => {
    fetchJson<{ runs: ScilHistoryRow[] }>('/api/scil')
      .then((data) => {
        setRuns(data.runs)
        setLoading(false)
      })
  ...
  if (!runs || runs.length === 0) {
    return (
      <div className="flex items-center justify-center h-64 text-[#4f4f4f]">
        No SCIL runs found. Run SCIL with: skillwalker scil --eval &lt;name&gt;
      </div>
    )
  }
  ...
  return (
    <div className="px-10 py-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-[#4f4f4f] text-[18px] mt-1.5">Track skill description improvement loop runs and accuracy</p>
      </div>
