...
export function ScilDetail(): JSX.Element {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await fetchJson<Partial<ScilRunDetails>>(`/api/scil/${runId}`)
  ...
  if (error)
    return (
      <div className="px-10 py-8">
        <Link to="/scil" className="flex items-center gap-2 text-[#4f4f4f] text-[18px] mb-4 hover:text-[#f0f0f0]">
          <span>&larr;</span>
  ...
  return (
    <div className="px-10 py-8">
      {/* Back link */}
      <Link to="/scil" className="flex items-center gap-2 text-[#4f4f4f] mb-6 w-fit hover:text-[#f0f0f0]">
