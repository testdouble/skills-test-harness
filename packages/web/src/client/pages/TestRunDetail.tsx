      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="bg-[#1a1b1a]">
            ...
            <th
              className="text-[#4f4f4f] text-[13px] font-bold text-left px-5 h-10 w-[80px] border-b border-[#252625]"
              style={{ letterSpacing: '1.5px' }}
            >
              RESULT
            </th>
          ...
          {criteria.map((cr, i) => (
            <Fragment key={i}>
              ...
              >
                ...
                <td
                  className={`text-[20px] font-bold px-5 w-[80px] ${
...
function CollapsibleOutputFile({ file }: { file: OutputFileRow }) {
  ...
  return (
    <div>
      ...
      {expanded && (
        <div className="bg-[#161716] border-x border-b border-[#2a2b2a]" style={{ borderRadius: '0 0 8px 8px' }}>
          ...
          {viewMode === 'rendered' ? (
            ...
            <pre className="px-7 py-6 text-[#c5c5c5] text-[13px] whitespace-pre-wrap overflow-x-auto">
              {file.fileContent}
            </pre>
          )}
        </div>
...
export function TestRunDetail(): JSX.Element {
  ...
  return (
    <div className="px-10 py-8">
      {/* Back link */}
      <Link to="/" className="flex items-center gap-2 text-[#4f4f4f] text-[18px] mb-6 w-fit hover:text-[#f0f0f0]">
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        {!allPassed && (
          <div className="flex items-center gap-1.5 px-4 py-2 rounded-md border border-[#d63c00] bg-[#1f1000]">
            ...
            <span className="text-[#d63c00] text-[16px] font-semibold">
              {failureCount} {failureCount === 1 ? 'Failure' : 'Failures'}
            </span>
          </div>
        )}
        {allPassed && (
          <div className="flex items-center gap-1.5 px-4 py-2 rounded-md border border-[#75fe04] bg-[#0d1a0d]">
            ...
            <tbody>
              {details?.summary.map((row, i) => (
                ...
                >
                  ...
                  <td
                    className={`text-[20px] font-bold px-5 ${row.all_expectations_passed ? 'text-[#75fe04]' : 'text-[#d63c00]'}`}
                  >
                    {row.all_expectations_passed ? '✓' : '✗'}
                  </td>
                  <td className="text-[#c5c5c5] text-[15px] px-5">${row.total_cost_usd.toFixed(3)}</td>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#1a1b1a]">
                ...
                <th
                  className="text-[#4f4f4f] text-[13px] font-bold text-left px-5 h-10 w-[100px] border-b border-[#252625]"
                  style={{ letterSpacing: '1.5px' }}
                >
                  RESULT
                </th>
            ...
            <tbody>
              {details?.expectations.map((row, i) => (
                ...
                >
                  ...
                  <td
                    className={`text-[20px] font-bold px-5 w-[100px] ${row.passed ? 'text-[#75fe04]' : 'text-[#d63c00]'}`}
                  >
                    {row.passed ? '✓' : '✗'}
                  </td>
                </tr>
        ...
        <div>
          ...
          {details.llmJudgeGroups.map((group, i) => (
            <div key={`${group.testName}-${group.rubricFile}-${i}`}>
              ...
              {details.outputFiles && <OutputFilesSection files={details.outputFiles} testName={group.testName} />}
            </div>
          ))}
        </div>
      )}
