...
const AGENT_STRIP_PATTERNS = AGENT_FIELDS_TO_STRIP.map(
...
export async function buildTempAgentPlugin(
  ...
  const override = overrideDescription != null
    ? { targetAgent: agentName, description: overrideDescription }
    : undefined

  await Promise.all([
    processAgents(pluginDir, tempDir, override),
    processSkills(pluginDir, tempDir)
  ])
