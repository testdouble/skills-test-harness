import type { TestSuiteConfig, RunTotals } from '@testdouble/harness-data'
...
export async function runTestCases(
  ...
  let current = await runPromptTests(skillPromptTests, config, suite, testSuiteDir, pluginDirs, debug, testRunId, totals, outputDir)
  current = await runSkillCallTests(skillCallTests, config, suite, testSuiteDir, debug, testRunId, current, outputDir, repoRoot)
  current = await runAgentCallTests(agentCallTests, config, suite, testSuiteDir, debug, testRunId, current, outputDir, repoRoot)
  current = await runAgentPromptTests(agentPromptTests, config, suite, testSuiteDir, pluginDirs, debug, testRunId, current, outputDir)
  return current
}
