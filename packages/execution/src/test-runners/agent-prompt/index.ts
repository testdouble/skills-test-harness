...
function printTestConfig(test: TestCase, plugins: string[]): void {
  process.stderr.write(`  - type: ${test.type ?? 'agent-prompt'}\n`)
...
export function wrapWithDelegation(agentName: string, promptContent: string): string {
  return `Use the ${agentName} agent to accomplish the following task:\n\n${promptContent}`
}

function parseAgentName(agentFile: string): string {
  const parts = agentFile.split(':')
  return parts.length > 1 ? parts[1] : agentFile
}

export async function runAgentPromptTests(
  tests: TestCase[],
  ...
  for (const test of tests) {
    ...
    const agentName = parseAgentName(test.agentFile!)
    const wrappedPrompt = wrapWithDelegation(agentName, promptContent)
