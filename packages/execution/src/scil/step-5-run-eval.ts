...
async function runSingleQuery(test: ScilTestCase, runIndex: number, opts: RunEvalOptions): Promise<QueryResult> {
  ...
  const actual = evaluateSkillCall(skillFile, true, events)
...
export async function runEval(opts: RunEvalOptions): Promise<QueryResult[]> {
  ...
  const results = new Array<QueryResult | undefined>(workItems.length)
  ...
  const completed = results.filter((r): r is QueryResult => r !== undefined)
