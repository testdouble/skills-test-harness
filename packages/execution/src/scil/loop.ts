...
export async function runScilLoop(config: ScilConfig): Promise<void> {
  ...
  const skill = await readSkill(skillMdPath)
  ...
  await writeSummaryOutput(runDir, runId, skill.description, iterations, best)
  ...
  if (best.description === skill.description) {
    ...
    await applyDescription(skillMdPath, best.description)
    process.stderr.write(`\nApplied best description to ${skillMdPath}\n`)
  } else {
    ...
    if (answer.trim().toLowerCase() === 'y') {
      await applyDescription(skillMdPath, best.description)
      process.stderr.write(`Applied best description to ${skillMdPath}\n`)
    }
  }
}
